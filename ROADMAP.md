# Maqraa Roadmap — Beyond the Live Session
...
The live session in Maqraa is functionally complete for a one-reciter-at-a-time tajweed/hifz halaqah:
...
This document captures what real Quran academies offer that Maqraa doesn't yet, organized by impact.
...
These are the features that move Maqraa from "solid live session tool" to "platform a teacher can build a practice on." Without them, the platform competes with Zoom + Mushaf app. With them, it competes with dedicated Quran academies that charge $30–80/month per student.
...
Maqraa already has all the data: `recitations` table tracks `student_id`, `surah`, `ayah_start`, `ayah_end`, `grade`, `created_at`, `turn_type`. From that, the platform can compute:
...
Real students practice between sessions. They need to know: "Memorize Al-Mulk verses 1–10 for next class." Right now Maqraa has no concept of an assignment between sessions — annotations are in-session only.
...
Maqraa has all the underlying data. What's missing is the rendering and delivery.
...
Maqraa's `StorageService` is scaffolded but unused. LiveKit Egress can record server-side audio.
...
Halaqat are sometimes conversational (the "dialogic halaqah" model). The Q&A pattern. Maqraa's one-reciter-at-a-time model is correct for tajweed/hifz, but for tilawa/muraja types a teacher might want a few minutes of open discussion at the end.
...
If we ever do this, it's a partnership move with established Quran academies — not something Maqraa builds alone.
...
## What this changes about Maqraa's positioning
